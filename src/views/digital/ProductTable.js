...
const ProductTable = ({ rows }) => {
  ...
  // ** header table
  const columns = [
    { field: 'product_id', headerName: 'Account Id', width: 130 },
    {
      field: 'product_name',
    ...
    },
    {
      ...
      renderCell: params => {
        return (
          <Button variant='contained' color='primary' onClick={() => handleApproveSubmit(params.row.product_id)}>
            approve
          </Button>
        )
      }
    },
    {
      ...
      renderCell: params => {
        return (
          <Button variant='contained' color='secondary' onClick={() => handleRejectSubmit(params.row.product_id)}>
            disapproved
          </Button>
        )
      }
  ...
  const handleApproveSubmit = id => {
    ...
    axios
      .put(`${process.env.NEXT_PUBLIC_API}DIGITAL.approve.productapprove`, data)
      .then(function (response) {
        // หลังจากที่อนุมัติสำเร็จ ลบแถวที่ถูก Approve ออกจากข้อมูล
        const updatedRows = tableRows.filter(row => row.product_id !== id)
  ...
  const handleRejectSubmit = id => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios
          .put(`${process.env.NEXT_PUBLIC_API}DIGITAL.approve.productreject`, data)
          .then(function (response) {
