...
const ServiceTable = ({ rows }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const Response = await axios.get(`${process.env.NEXT_PUBLIC_API}DIGITAL.approve.all_ser_servicename`, {
          headers: {
  ...
  const handleApproveSubmit = (reqId, SerID) => {
    ...
    axios
      .put(`${process.env.NEXT_PUBLIC_API}DIGITAL.approve.requirement_ser_approve`, data)
  ...
  const handleRejectSubmit = reqId => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios
          .put(`${process.env.NEXT_PUBLIC_API}DIGITAL.approve.requirement_ser_reject`, data)
          .then(function (response) {
