...
const ProductTable = ({ rows }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const Response = await axios.get(`${process.env.NEXT_PUBLIC_API}DIGITAL.approve.allsuppliername`, {
          headers: {
  ...
  const handleApproveSubmit = (reqId, subId) => {
    ...
    axios
      .put(`${process.env.NEXT_PUBLIC_API}DIGITAL.approve.requirementapprove`, data)
