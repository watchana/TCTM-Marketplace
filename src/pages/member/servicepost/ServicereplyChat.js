...
const ServicereplyChat = serId => {
  ...
  // ** Data Grid Columns
  const columns = [
    { field: 'ser_req_id', headerName: 'ID  ', width: 90 },
    { field: 'ser_req_header', headerName: 'Title', width: 250 },
    {
      field: 'creation',
    ...
    },
    {
      ...
      renderCell: params => {
        const reqStatus = params.row.ser_req_status
        if (reqStatus === '2') {
    ...
    },
    {
      ...
      renderCell: rowCell => {
        const handleDetailClick = () => {
          router.push(
            `/member/servicepost/ServiceReplyDetail/?ser_req_id=${rowCell.row.ser_req_id}&ser_id=${ser_id}&member_id2=${rowCell.row.member_id}`
          )
        }
  ...
  const getUniqueReqStatus = () => {
    ...
    const uniqueReqStatus = [...new Set(rowdata.map(row => row.ser_req_status))]
          ...
          <Box sx={{ display: 'flex', flexDirection: 'row', paddingLeft: 1 }}>
            <Typography variant='body1' fontWeight='' fontSize='1.0rem' color='#000'>
              {(rowdata &&
                rowdata.filter(
                  row =>
                    (searchTerm === '' || row.ser_req_header.toLowerCase().includes(searchTerm.toLowerCase())) &&
                    (selectedReqStatus === '' || row.ser_req_status === selectedReqStatus)
                ).length) ||
                '0 '}
              Requirements
            </Typography>
...
export default ServicereplyChat
