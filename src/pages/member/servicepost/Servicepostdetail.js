...
const ServiceDetail = () => {
  ...
  const { ser_req_id } = router.query
  const serID = ser_req_id
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!serID) {
        // console.error('serID is undefined or null')
      ...
      try {
        ...
        if (!response.data.message.image) {
          ...
          setInformationImg(response.data.message.image)
          setInformationData(response.data.message.Requirement_Data[0])
        }
      } catch (error) {
        console.error(error)
      }
            ...
            <Grid item xs={12}>
              ...
              >
                ...
                <CardMedia
                  component='img'
                  image={
                    informationimg[stateImages]?.ser_req_image_file
                      ? `/serimgpost/${informationimg[presentState].ser_req_image_file}`
                      : ''
                  }
                  alt={informationdata.ser_req_header}
                  height='100%'
                  sx={{ width: '90%', objectFit: 'contain' }}
                  title={informationdata.ser_req_header}
                />
                <MySeo
                  title={informationdata.ser_req_header} // details={OptionData}
                  description={Seoinformationpage.description} // content={SeoProductpage.content}
                    ...
                    >
                      {informationimg.slice(firstImage, endImage).map((image, index) => (
                        ...
                        >
                          <CardMedia
                            component='img'
                            src={`/serimgpost/${image.ser_req_image_file}`}
                            alt={`Image ${index + 1}`}
      ...
      <Grid container spacing={3}>
        <Grid item xs={12}>
          <Box sx={{ display: 'flex', justifyContent: 'center', width: '100%', mt: 20, mb: 6 }}>
            ...
            >
              {informationdata.ser_req_header}
            </Typography>
          </Box>
        </Grid>
        ...
        <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'center' }}>
          <Typography variant='body1' fontSize='16px' color='#606060' style={{ whiteSpace: 'pre-line', width: '80%' }}>
            {informationdata.ser_req_description}
          </Typography>
        </Grid>

        <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'center' }}>
          <Typography variant='body1' fontSize='16px' color='#606060' style={{ whiteSpace: 'pre-line', width: '80%' }}>
            Post by {informationdata.user_first_name}
          </Typography>
        </Grid>
      </Grid>
    </Box>
