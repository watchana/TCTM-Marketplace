      ...
      >
        <MenuItem disableRipple>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
            ...
            <Chip
              size='small'
              label={`${notificationData.Noread} New`}
              color='primary'
        ...
        <ScrollWrapper>
          {notificationData.Data && notificationData.Data.length > 0 ? (
            notificationData.Data.map((item, index) => (
              <MenuItem key={item.noti_id} onClick={handleDropdownClose}>
                <Link href={item.link_url} passHref>
                  <a target='_blank' style={{ textDecoration: 'none' }}>
                    <Box sx={{ width: '100%', display: 'flex', alignItems: 'center' }}>
                      <Avatar alt='message' src='/images/avatars/5.png' />
                      <Box sx={{ mx: 4, flex: '1 1', display: 'flex', overflow: 'hidden', flexDirection: 'column' }}>
                        <MenuItemTitle>{item.title}</MenuItemTitle>
                        <MenuItemSubtitle variant='body2'>{item.creation}</MenuItemSubtitle>
                      </Box>
                    </Box>
                  </a>
                </Link>
